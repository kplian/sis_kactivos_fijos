...
BEGIN
	...
	***********************************/
	...
	if(p_transaccion='SKA_AFCCOSTO_INS')then
        ...
        begin
        v_horas:=(select sum(cantidad_horas) from kaf.tactivo_fijo_cc where id_activo_fijo=v_parametros.id_activo_fijo
        and estado_reg='activo'
        );
	if((v_horas+v_parametros.cantidad_horas)>720) then
    	raise exception 'Esta ingresando % horas y ya se tiene registrado % horas.El maximo para el activo es 720',v_parametros.cantidad_horas,v_horas;
    end if;
	...
	***********************************/
        ...
        if((v_horas-(select cantidad_horas from kaf.tactivo_fijo_cc where id_activo_fijo_cc=v_parametros.id_activo_fijo_cc  )+v_parametros.cantidad_horas)>720) then
    	   raise exception 'Esta ingresando % horas y ya se tiene registrado % horas adicional a este registro.El maximo para el Activo es de 720',v_parametros.cantidad_horas,(v_horas-(select cantidad_horas from kaf.tactivo_fijo_cc where id_activo_fijo_cc=v_parametros.id_activo_fijo_cc  ));
    	end if;
	...
	***********************************/
        ...
        begin
	        ...
	        if (((select coalesce(sum(cantidad_horas),0) from kaf.tactivo_fijo_cc where id_activo_fijo=v_id_activo_fijo and estado_reg='activo')+ coalesce(v_parametros.cantidad_horas,0))>720  ) then
               v_mensaje:=v_mensaje|| 'La cantidad de horas a subir mas las ya registradas, para '||v_parametros.activo_fijo||' supera el maximo permitido (720)';
            ...
            if(v_mensaje!='') then 
                ...
                for v_registros in (select id_activo_fijo, sum(cantidad_horas) as cantidad_horas from kaf.tactivo_fijo_cc where extract (month from mes)=v_mes
            						and extract (year from mes)=v_gestion and estado_reg='activo'
                                    group by id_activo_fijo
                                    having  sum(cantidad_horas)<720) loop
                    v_codigo_activo:=  (select codigo from kaf.tactivo_fijo where id_activo_fijo=v_registros.id_activo_fijo );              
                	v_mensaje:=('- Activo: ' ||v_codigo_activo || ' solo tiene '|| v_registros.cantidad_horas || ' horas asignadas. Debe completar a 720 para el periodo '||v_mes);  
